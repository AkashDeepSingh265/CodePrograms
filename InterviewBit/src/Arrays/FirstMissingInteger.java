...
public class FirstMissingInteger {
    public static int firstMissingPositive(List<Integer> A) {
        ...
        for(Integer i : A){
            int exact_value = (i-1)%(max+1);
            if((i-1 >=0 ) && (exact_value < A.size())){
                System.out.println(i+" ");
                if(A.get(exact_value)>=0){
                    A.set(exact_value,A.get(exact_value)+max+1);
                }else{
                    A.set(exact_value,A.get(exact_value)-max-1);
                }
            }
